...
from nets.swinunet_training import (get_lr_scheduler, set_optimizer_lr,
                                 weights_init)
...
if __name__ == "__main__":
    ...
    model = SwinUnet(img_size=input_shape[0], patch_size=4, in_chans=3, num_classes=num_classes, embed_dim=96,
                     depths=[2, 2, 6, 2],
                     num_heads=[3, 6, 12, 24], window_size=7, mlp_ratio=4., qkv_bias=True, qk_scale=None, drop_rate=0.0,
                     drop_path_rate=0.1, ape=False, patch_norm=True, use_checkpoint=False, zero_head=False, vis=False)
    ...
    if True:
        ...
        train_dataset   = SegmentationDataset(train_lines, input_shape, num_classes, True, VOCdevkit_path, "train", True)
        val_dataset     = SegmentationDataset(val_lines, input_shape, num_classes, False, VOCdevkit_path, "val", False)
